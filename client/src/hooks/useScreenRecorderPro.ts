...
export function useScreenRecorderPro() {
  // Internal state
  ...
  const startTimer = useCallback(() => {
    ...
    // Set the start time, accounting for any paused time
    startTimeRef.current = Date.now() - (pausedTimeRef.current || 0);
    
    // Update immediately to ensure timer shows right away
    const initialElapsedTime = Math.floor((Date.now() - startTimeRef.current) / 1000);
    setState(prev => ({
      ...prev,
      recordingTime: initialElapsedTime
    }));
    
    // Then set the interval to update continuously
  ...
  const stopRecording = useCallback(() => {
    if (state.mediaRecorder && (state.isRecording || state.isPaused)) {
      try {
        ...
        // Stop the MediaRecorder (this will trigger ondataavailable and onstop)
  ...
  }, [state.recordingMode, state.settings, state.isRecording, state.mediaStream, getVideoConstraints, setupVideoPreview, stopRecording]);
  ...
  const startRecording = useCallback(async (skipPermissionRequest: boolean = false) => {
    try {
      ...
      // Set up recorder state before starting to ensure we capture all data
      setState(prev => ({
        ...prev,
      ...
      // Start the recorder before we begin the timer
      // Request data every 1 second for progress updates
      recorder.start(1000);
      console.log("MediaRecorder started successfully");
      
      // Then start the timer after MediaRecorder is fully started
      startTimer();
      ...
      // This state was already set above, so we don't need to set it again
